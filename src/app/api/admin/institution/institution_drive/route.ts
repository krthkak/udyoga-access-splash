...
export async function GET(req: Request) {
  try {
    ...
    const instDrive = await prisma.institutionDrive.findUnique({
      where: { id },
    });
    ...
    return NextResponse.json({ institutionDrive: instDrive }, { status: 200 });
  } catch (err) {
    console.error(
      "Error in GET /api/admin/institution/institution_drive:",
      err
    );
...
export async function POST(req: Request) {
  try {
    ...
    if (action === "update") {
      ...
      const updated = await prisma.institutionDrive.update({
        where: { id },
        data,
      });

      return NextResponse.json(
        { message: "success", institutionDrive: updated },
        { status: 200 }
      );
    }
    ...
    console.error(
      "Error in POST /api/admin/institution/institution_drive:",
      err
    );
    return NextResponse.json(
      { error: "Internal server error" },
